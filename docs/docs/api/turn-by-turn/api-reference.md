...
| `date_time` | Local date and time for the route. Supported types for `/route`:<ul><li>`0` — **Current** departure time (`value` ignored; server uses now).</li><li>`1` — **Specified departure time** (`depart_at`).</li><li>`2` — **Specified arrival time** (`arrive_by`). Not yet implemented for multimodal costing.</li><li>`3` — **Invariant** specified time — **not supported on `/route`**. Requests are rejected with HTTP 400 and internal error **169** (`Invariant date_time is not supported for route requests`).</li></ul>`value` — ISO 8601 format (`YYYY-MM-DDThh:mm`) in the local time zone of departure or arrival, e.g. `"2016-07-03T08:06"`.<br><br>**Algorithm selection (motorized `/route`):** no `date_time` → bidirectional A\*; `current` / `depart_at` → **TDALT** when `thor.tdalt.enabled` and landmark data are available, otherwise `TimeDepForward` if `thor.tdalt.fallback_to_unidirectional` is true; **`arrive_by` → `TimeDepReverse` (not TDALT)**. See [path algorithms](../../thor/path-algorithm.md#time-dependent-routing). |
...
| `prioritize_bidirectional` | **Deprecated for time-dependent routing.** When `date_time.type` is `current` or `depart_at`, Thor selects TDALT (or unidirectional time-dependent A\* as fallback) — not bidirectional A\* with post-hoc recost. This option only affects legacy behavior when no TD algorithm applies. |
...
| `reverse_time_tracking` (**BETA**) | Which time tracking strategy to use on the non-time aware expansion in the bidirectional routing algorithm: `disabled` will assume no time, `heuristic` will try a best guess based on the bee line distance between origin and destination. Default `heuristic`. **Note:** Obsolete for time-dependent `/route` requests — TDALT and unidirectional TD algorithms handle time during search. |
...
|169 | Invariant date_time is not supported for route requests |
